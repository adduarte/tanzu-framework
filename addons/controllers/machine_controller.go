...
func (r *MachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	clusterBootstrap, err := MachineToClusterBoostrap(ctx, machine, r.Client)
...
// is absent
func (r *MachineReconciler) reconcileMachineDeletionHook(ctx context.Context, log logr.Logger, obj *clusterv1.Machine, clusterBootstrap *runtanzuv1alpha3.ClusterBootstrap) (ctrl.Result, error) {
	log.Info("Start reconciling machine deletion pre-terminate hook")

	res := ctrl.Result{}

	if ContainsFinalizer(clusterBootstrap, AdditionalPackageFinalizer) { // TODO (adduarte) ask: do we add finalizer to clusterboostrap or clusterBootstrap object.
